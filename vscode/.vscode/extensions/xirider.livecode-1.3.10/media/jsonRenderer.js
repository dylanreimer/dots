...
var module, window, define, renderjson = (function () {
    ...
    function _renderjson(json, indent, dont_indent, show_level, options, first_level) {
        ...
        if (first_level == true) {
            ...
            // put disclosure here maybe
            json = json.abcdict;

            var nlines = json.nlines + 1;

            return undisclosure("", options.collapse_msg(json.length), "", "object", function () {
                    var os = append(span("object"), themetext("object syntax", "", null, ""));

                    for (var lineid = 0; lineid < nlines; lineid++) {

                        if (lineid in json) {
                            // add var to line
                            var line = json[lineid];
                            
                            for (var linevarid = 0; linevarid < line.length; linevarid++) {

                                var singlevar = line[linevarid]

                                if (isempty(singlevar, options.property_list))
                                    return themetext(null, my_indent, "object syntax", "{}");

                                // need to put disclosere func on the highest level and return at the end


                                // for (var k in singlevar) var last = k;
                                // var keys = options.property_list || Object.keys(singlevar);
                                // if (options.sort_objects)
                                //     keys = keys.sort();
                                // for (var i in keys) {
                                //     var k = keys[i];

                                // if (!(k in singlevar)) continue;
                                var keys = Object.keys(singlevar);
                                var key = keys[0];



                                if (linevarid == 0) {
                                append(os, themetext(null, "", "key", key, "object syntax", ': '),
                                _renderjson(options.replacer.call(singlevar, key, singlevar[key]), "", true, show_level - 1, options, false),
                                linevarid != line.length ? themetext("syntax", " ") : [],
                                // removed comma
                                text(" "));
                                } else {
        ...
        if (json.constructor == Array) {
            ...
            return disclosure("[", options.collapse_msg(json.length), "]", "array", function () {
                ...
                for (var i = 0; i < json.length; i++)
                    append(as,
                        _renderjson(options.replacer.call(json, i, json[i]), "", false, show_level - 1, options, false),
                        //reduced indent
                        i != json.length - 1 ? themetext("syntax", ", ") : [],
                        text(""));
                        //removed \n
                append(as, themetext(null, "", "array syntax", "]"));
                return as;
            });
        }
        ...
        return disclosure("{", options.collapse_msg(Object.keys(json).length), "}", "object", function () {
            ...
            for (var i in keys) {
                ...
                append(os, themetext(null, "", "key", k, "object syntax", ': '),
                    _renderjson(options.replacer.call(json, k, json[k]), "", true, show_level - 1, options, false),
    ...
    var renderjson = function renderjson(json) {
        ...
        var pre = append(document.createElement("pre"), _renderjson(json, "", false, options.show_to_level, options, true));
